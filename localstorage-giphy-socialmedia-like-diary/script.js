...
let createPost = () => {
    ...
    if (spotify.length > 0) {
        let urlContent = spotify.split("/");
        spotify = urlContent[urlContent.length-2] + "/" + urlContent[urlContent.length-1].split("?")[0];
    }
...
let addIntoTimeline = (post) => {
    ...
    let postBody = `<div class="bg-body-tertiary p-3 rounded" data-id="${post.id}">
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    <button class="btn btn-outline-secondary btn-sm my-auto" onclick="comment(${post.id})">
                                        <i class="bi bi-chat-left-text"></i>
                                    </button>
                                    <button class="btn btn-outline-danger btn-sm my-auto" onclick="remove(${post.id})">
                                        <i class="bi bi-trash3"></i>
                                    </button>
                                </div>
                            </div>
                            <p class="mt-2 fw-medium">${post.conteudo}</p>
                            ${mediaEmbed}
                            ${spotifyEmbed}
                        </div>`
